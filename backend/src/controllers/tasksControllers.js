...
export const createTask = async(req, res) => {
    try {
        ...
        console.error("loi khi goi crateTask", error);
        res.status(500).json({ message: "loi server" });
    }
}
...
export const updateTask = async(req, res) => {
    try {
        ...
        const updatedAt = await Task.findByIdAndUpdate(
           req.params.id,
           {
            title,
            status,
        ...
        console.error("loi khi goi uodateTask", error);
        res.status(500).json({ message: "loi server" });
    }
} 
...
export const deleteTask = async(req, res) => {
    try {
        ...
        if(!deleteTask){
            return res.status(404).json({message: "Nhiem vu khongton tai"})
        }
